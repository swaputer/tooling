# SwapVM v1.0 — Frozen Buy-to-Execute Protocol Specification
...
The actor is recovered from the signature, never inferred from the Hook callback sender. Nonces are sequential per `(worldId, actor)` and commit only on success.
...
Child failure propagates to the root. v1.0 has no catchable child revert or partial journal.
...
36. EIP-712 signatures fail when any pool, payload, input, recipient, router, executor, price-limit, nonce, deadline or gas field changes.
...
Root authentication:    EIP-712 low-s EOA signatures
